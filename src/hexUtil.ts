...
for (let index = 0; index < planetCount; index++) {    
    let angle = -1 * index * Math.PI / 1.5;
    planetCenters.push(new Vector3(planetSpreadRadius * Math.sin(angle), 0, planetSpreadRadius * Math.cos(angle)));
}
...
const tileCoordsTo3d = (col: number, row: number, planet: number | string | undefined): Vector3 => {
    let planetIndex = 0;
