...
import { ArcRotateCamera, Camera, Color3, DirectionalLight, EventState, Vector3, Viewport } from "@babylonjs/core";
import { resources } from "./resourceMeshes";
import { AdvancedDynamicTexture } from "@babylonjs/gui/2D/advancedDynamicTexture";
import { Control, TextBlock } from "@babylonjs/gui";

const createKeyScene = (engine: Engine, mainCamera: ArcRotateCamera, resourceName: string, options?:{row?:number, scale?:number}): Scene => {
    const scene = new Scene(engine);
    scene.autoClear = false;
    const camera = new ArcRotateCamera("Camera", -5 * Math.PI / 8, Math.PI / 3, 10, Vector3.Zero(), scene);
    let viewportY = 0.88;
    if (options?.row) {
